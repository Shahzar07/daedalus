...
description: Modify, debug, or extend the s6-overlay supervision tree inside the Daedalus Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.
tags: [docker, s6, supervision, gateway, profiles]
---

# Daedalus s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the Daedalus Docker image (something that should be supervised at every container start, like the dashboard)
...
If you're just running the Daedalus and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│       └── walk $DAEDALUS_HOME/profiles/<name>/gateway_state.json
...
│   └── dashboard/run                  ← if DAEDALUS_DASHBOARD=1, runs `hermes dashboard`
│
...
│   │   └── log/run     (s6-log → $DAEDALUS_HOME/logs/gateways/coder/current)
│   └── ...
│
...
| `docker/s6-rc.d/dashboard/run` | Conditional service — `exec sleep infinity` unless `DAEDALUS_DASHBOARD` is truthy. |
...
1. **cont-init.d scripts receive no CMD args** — so the stage2 hook can't parse `docker run <image> chat -q "hi"` to set `DAEDALUS_ARGS` for a service `run` script to consume.
...
DAEDALUS_TEST_IMAGE=hermes-agent-harness:latest scripts/run_tests.sh tests/docker/ -v
...
The cont-init reconciler runs as hermes (`s6-setuidgid hermes` in `02-reconcile-profiles`). If a profile dir ends up root-owned (e.g. because `docker exec <c> hermes profile create …` ran as root by default), the reconciler can't read SOUL.md and fails with `PermissionError`. Mitigation: `stage2-hook.sh` chowns `$DAEDALUS_HOME/profiles` to hermes on **every** boot, idempotently. Don't remove that block.

### Files written by `docker exec` are root-owned

`docker exec` defaults to root. Either pass `--user hermes` or rely on the stage2 chown sweep next reboot. Don't write files under `$DAEDALUS_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
...
- `hermes-tool-quirks`: Specific Daedalus-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with hermes built-in tools.
