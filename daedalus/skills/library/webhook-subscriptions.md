...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Daedalus agent runs by POSTing events to a URL.
...
- Subscriptions persist to `~/.dae/webhook_subscriptions.json`

## How It Works

1. `hermes webhook subscribe` writes to `~/.dae/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.dae/logs/gateway.log | tail -20`
