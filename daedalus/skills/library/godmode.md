...
exec(open(os.path.expanduser(
    os.path.join(os.environ.get("DAEDALUS_HOME", os.path.expanduser("~/.dae")), "skills/red-teaming/godmode/scripts/load_godmode.py")
)).read())
...
1. **Reads `~/.dae/config.yaml`** to detect the current model
...
7. **If a strategy works**, locks it in:
   ...
   - Writes prefill messages to `~/.dae/prefill.json`
...
exec(open(os.path.join(os.environ.get("DAEDALUS_HOME", os.path.expanduser("~/.dae")), "skills/red-teaming/godmode/scripts/parseltongue.py")).read())
...
```python
# Via execute_code
exec(open(os.path.join(os.environ.get("DAEDALUS_HOME", os.path.expanduser("~/.dae")), "skills/red-teaming/godmode/scripts/godmode_race.py")).read())
...
5. **Daedalus models don't need jailbreaking** — nousresearch/hermes-3-* and hermes-4-* are already uncensored. Use them directly for the fastest path.
...
12. **execute_code sandbox has no env vars** — When Daedalus runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.dae/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.dae/.env"))`
