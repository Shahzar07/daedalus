...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.dae/.env ] && grep -q "^GITHUB_TOKEN=" ~/.dae/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.dae/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Daedalus\",
    \"comments\": [
...
```bash
source "${DAEDALUS_HOME:-$HOME/.dae}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Daedalus. Code looks clean — good test coverage, no security concerns."
